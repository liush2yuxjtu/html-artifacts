...
if [ -n "$current" ]; then
  msg="git hooks: core.hooksPath is '$current', so .githooks/post-merge (ChatCut v3 compare preview) does not run. Do not override it; ask the human whether to call .githooks/post-merge from their own hook."
else
  msg="git hooks: not installed in this clone, so .githooks/post-merge (rebuild + serve the ChatCut v3 compare page after git pull) does not run. Soft gate: run 'sh chatcut-v3/scripts/install-hooks.sh' (reversible with 'git config --unset core.hooksPath') and tell the human you did, or tell them to run it."
fi

...
export function extractAssetUrls(html, pageUrl) {
  ...
  for (const match of decoded.matchAll(/(?:src|poster|href)\s*=\s*["']([^"']+)["']/gi)) consider(match[1]);
  for (const match of decoded.matchAll(/srcset\s*=\s*["']([^"']+)["']/gi)) {
    for (const part of match[1].split(',')) consider(part.trim().split(/\s+/)[0]);
  }
  for (const match of decoded.matchAll(/url\(\s*["']?([^"')]+)["']?\s*\)/gi)) consider(match[1]);
  for (const match of decoded.matchAll(/https?:\/\/[^\s"'<>]+/gi)) consider(match[0]);
  for (const match of decoded.matchAll(/(?:^|[\s"'(=:])((?:\/[A-Za-z0-9._~!$&()*+,;=:@%\/-]+)\.(?:avif|gif|jpe?g|png|svg|webp|mp4|webm|mov|m4v|mp3|wav|m4a|ogg|aac|css|m?js)(?:\?[^\s"'<>)]+)?)/gim)) consider(match[1]);

  return { media: [...media].sort(), stylesheets: [...stylesheets].sort(), scripts: [...scripts].sort() };
}
...
function shouldRemoveScript(tag) {
  ...
  const trackingMarkers = ['googletagmanager.com','google-analytics.com','posthog','ahrefs.com/analytics','firstpromoter','cloudflareinsights.com','beacon.min.js','gtm-','auth/get-session'];
  return trackingMarkers.some((marker) => lower.includes(marker));
}

export function sanitizeHtml(html) {
  return html.replace(/<script\b[^>]*>[\s\S]*?<\/script\s*>/gi, (tag) => shouldRemoveScript(tag) ? '' : tag);
}
...
export function rewritePageLinks(html) {
  ...
  const rewrite = (value, attr) => {
    ...
    let url;
    try { url = new URL(decoded, ORIGIN + '/'); } catch { return value; }
    if (url.origin !== ORIGIN) return value;
    if (attr.toLowerCase() === 'href') {
      if (url.pathname === '/' || url.pathname === '/features' || /^\/features\/[^/]+\/?$/.test(url.pathname)) return `${url.pathname}${url.search}${url.hash}`;
      if (/\.(?:css|m?js)$/i.test(url.pathname)) return url.toString();
      return url.toString();
    }
    return url.toString();
  };
  for (const attr of ['href','src','poster']) out = rewriteAttrUrl(out, attr, rewrite);
  out = out.replace(/srcset\s*=\s*["']([^"']+)["']/gi, (all, value) => {
    const quote = all.includes('="') ? '"' : "'";
    const rewritten = value.split(',').map((part) => { const bits = part.trim().split(/\s+/); bits[0] = rewrite(bits[0], 'src'); return bits.join(' '); }).join(', ');
...
export function rewriteMediaUrls(html, urlToLocalPath) {
  ...
  for (const [remote, local] of entries) {
    const variants = new Set([remote, remote.replaceAll('/', '\\/'), remote.replaceAll('&', '&amp;')]);
...
export function injectHomepagePatch(html) {
  let out = html;
  if (!out.includes('/patches/home.css')) out = out.replace(/<\/head\s*>/i, '<link rel="stylesheet" href="/patches/home.css"></head>');
  if (!out.includes('/patches/home.js')) out = out.replace(/<\/body\s*>/i, '<script type="module" src="/patches/home.js"></script></body>');
  return out;
}

export const mirrorInternals = { ORIGIN, MEDIA_EXT_RE, FONT_EXT_RE, CSS_EXT_RE, JS_EXT_RE, decodeHtmlEntities, toAbsolute };
